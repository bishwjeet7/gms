...
exports.updateAnnouncement = async (req, res) => {
    console.log("updateAnnouncement route hit");
    try {
        let data = { ...req.body };
        ...
        delete data.announcement_id; // Remove announcement_id from data as it's not part of the update data

        const update = { $set: data };
        // Use Announcement model instead of announcement_id
        const updateResult = await Announcement.updateOne(query, update);

        if (updateResult.nModified === 0) {
            return res.status(404).json({ error: 'No announcement found to update.' });
        }
